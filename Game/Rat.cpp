...
Rat::Rat()
{
	intro = "\n'Good morning my son!' cries a voice from the clearing. A large rat wearing a tacky green suit appears, holding a platter of cake!\n";
	challenge = "\nMmm! Are you having cake?!?\nThe rat seems to think you have more cake in your inventory. It's unclear what will happen if you don't give it to him...\n";
	fightOptions = "\nA. Ignore Mr. Ratburn\tB. Offer Mr. Ratburn some of your food\n C. Hit Mr. Ratburn";
}
...
int Rat::fightBoss(Player player) {
	...
	if (response == "a") {
		std::cout << "\nYou attempt to ignore Mr. Ratburn.\nBefore you know it he's right in front of you with those glowing red eyes! As you look into them you're turned into stone!";
		std::cout << "\nYou are KILLED!";
		return 100;
	}
	else if (response == "b") {
		std::cout << "\nYou offer Mr. Ratburn some of your food. It's unlikely this will be very effective unless you have cake.";
		if (player.inventoryContains("cake", 1)) {
			std::cout << "\n'You made cake?? For ME?!'\nMr. Ratburn is thrilled that you had cake to give him and decides to let you go!";
			int total = player.checkqty("cake");
			if (total > 1) {
				std::cout << "\nYou notice Mr. Ratburn has somehow not only eaten the one slice you offered him but has taken all the cake in your inventory!";
